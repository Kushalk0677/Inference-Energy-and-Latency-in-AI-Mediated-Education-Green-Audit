...
MAX_NEW_TOKENS = 200
USE_QUANTIZATION = True   # False = FP16 (Run 1) | True = NF4 (Run 2)
...
# Phi-3-mini uses rope_scaling with "rope_type" but modeling_phi3.py expects
# "type". Setting rope_scaling=None uses standard unscaled RoPE, which is
# correct for the 4k native context version.
...
# ── Pre-load patch ─────────────────────────────────────────────────────────────
# modeling_phi3.py calls .to() internally during __init__, BEFORE we get a
# handle on the model object — so patching after from_pretrained is too late.
# We patch modeling_utils.PreTrainedModel.to() globally beforehand so that any
# .to() call on an already-quantized model is silently ignored instead of
# raising ValueError.
import transformers.modeling_utils as _mu
_original_pretrained_to = _mu.PreTrainedModel.to

def _patched_pretrained_to(self, *args, **kwargs):
    from transformers.utils.quantization_config import QuantizationMethod
    if getattr(self, "quantization_method", None) == QuantizationMethod.BITS_AND_BYTES:
        return self   # no-op — bitsandbytes already placed everything correctly
    return _original_pretrained_to(self, *args, **kwargs)

_mu.PreTrainedModel.to = _patched_pretrained_to
# ──────────────────────────────────────────────────────────────────────────────
...
# ==============================================================================
# One tracker with manual energy snapshots per prompt avoids per-prompt tracker
# drift and matches the approach in kvcache_50prompts_true.csv.
